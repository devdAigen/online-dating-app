...
public class ChatActivity extends AppCompatActivity {
    ...
    DatabaseReference mDatabseUser,mDatabseChat,anouser;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Toast.makeText(this, "currentuid "+currentUserId+" matchid "+MatchId, Toast.LENGTH_SHORT).show();
        mDatabseUser=FirebaseDatabase.getInstance().getReference().child("Users").child(currentUserId).child("connections").child("match").child(MatchId).child("ChatId");
        ...
        mChatAdapter=new ChatAdapter(getChatDataSet(),ChatActivity.this);
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if(dataSnapshot.exists()) {
                    if (dataSnapshot.getValue() != null) {
                     ...
                     anouser=FirebaseDatabase.getInstance().getReference().child("Users").child(currentUserId).child("anonymous").child(MatchId);

                    anouser.addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            String key;
                            if(dataSnapshot.exists()){
                                if(dataSnapshot.getValue()!=null){
                                    key=dataSnapshot.child("ChatId").getValue().toString();
                                   }else{
                                    key=FirebaseDatabase.getInstance().getReference().child("Chat").push().getKey();
                                    anouser.child("ChatId").setValue(key);
                                    userdb.child(MatchId).child("anonymous").child(currentUserId).child("ChatId").setValue(key);
